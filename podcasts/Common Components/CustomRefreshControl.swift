...
class CustomRefreshControl: UIRefreshControl {
    ...
    // Pull-to-refresh glyph. Upstream refresh_inner / refresh_outer are the
    // Pocket Casts spinner mark; white-label builds substitute neutral
    // concentric ring-arcs (generated by scripts/generate_whitelabel_refresh.py).
    private enum Asset {
        #if WHITELABEL
        static let inner = "refresh_inner_whitelabel"
        static let outer = "refresh_outer_whitelabel"
        #else
        static let inner = "refresh_inner"
        static let outer = "refresh_outer"
        #endif
    }
    ...
    private func setupView() {
        ...
        refreshInnerImage.image = UIImage(named: Asset.inner)?.withRenderingMode(.alwaysTemplate)
        refreshInnerImage.alpha = 0
        refreshInnerImage.translatesAutoresizingMaskIntoConstraints = false
        addSubview(refreshInnerImage)

        refreshOuterImage.image = UIImage(named: Asset.outer)?.withRenderingMode(.alwaysTemplate)
